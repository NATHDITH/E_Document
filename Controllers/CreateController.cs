...
.Add(new Text($"{(model.Maintenancemoney.HasValue && model.Maintenancemoney > 0 ? model.Maintenancemoney.Value.ToString("N2") : "N/A")} บาท").SetFont(thaiFont))
    ...
    ;
        ...
        //        [HttpPost]
        //        public IActionResult GenerateDisbursePdf([FromForm] DocumentDetail2 model)
        //        {
        //            using (MemoryStream stream = new MemoryStream())
        //            {
        //                PdfWriter writer = new PdfWriter(stream);
        ...
        //                string formattedDatePri = model.ProjectPrinDate?.ToString("ddddที่ d MMMM yyyy", new System.Globalization.CultureInfo("th-TH")) ?? "N/A";
        //                document.Add(new Paragraph("วันที่ ")
        //                    .Add(new Text(formattedDatenow).SetFont(thaiFont))  // แสดงวันที่ที่แปลงแล้ว
        //                    .SetFont(thaiFont)
        //                    .SetFontSize(12)
        ...
        //    .Add(new Text(formattedDatePri).SetFont(thaiFont)) // ✅ แปลงเป็น string พร้อม formatting
        //    .Add(new Text(" เรื่อง ขออนุมัติหลักการและวงเงินค่าใช้จ่ายโครงการ  ").SetFont(thaiFont)
        //    ).Add(new Text(model.ProjectName ?? "N/A").SetFont(thaiFont))
        ...
        //    .Add(new Text(" ตามแผนการใช้เงินบำรุงกิจกรรมนิสิต ปีการศึกษา 2568 โดยมีวงเงินค่าใช้จ่ายจำนวน  ").SetFont(thaiFont)).Add(new Text($"{(model.ProjectCost > 0 ? model.ProjectCost.ToString("N2") : "N/A")} บาท")
        //    .SetFont(thaiFont))
        ...
        //        [HttpPost]
        //        public async Task<IActionResult> GeneratePrinciplesPdf([FromForm] DocumentDetail2 model)
        //        {

        //            var documentDetail = new DocumentDetail2
        //            {
        //                DocNo = model.DocNo,
        //                CreateDate = model.CreateDate,
        //                ProjectName = model.ProjectName,
        ...
        //            _context.DocumentDetails2.Add(documentDetail);
        //            await _context.SaveChangesAsync();  // บันทึกข้อมูลลงใน DocumentDetail
        ...
        //            Console.WriteLine($"Maintenancemoney: {model.Maintenancemoney}");
        ...
        //                    .Add(new Text(" / 2568 ").SetFont(thaiFont)) // ข้อความ "/2567"
        //                );
        //                string formattedDatenow = model.CreateDate?.ToString("dd MMMM yyyy", new System.Globalization.CultureInfo("th-TH")) ?? "N/A";
        //                string formattedDatestart = model.StartDate?.ToString("ddddที่ d MMMM yyyy", new System.Globalization.CultureInfo("th-TH")) ?? "N/A";
        //                string formattedDateend = model.EndDate?.ToString("ddddที่ d MMMM yyyy", new System.Globalization.CultureInfo("th-TH")) ?? "N/A";
